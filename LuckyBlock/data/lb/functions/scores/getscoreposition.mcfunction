execute at @p as @a[scores={abandon=0},gamemode=spectator] unless score @s currentplayer = @p currentplayer if score @s timerun <= @p timerun run scoreboard players add @p Position 1
