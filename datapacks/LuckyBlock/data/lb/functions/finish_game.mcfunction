...
execute unless score @e[name="GameManager",limit=1,type=armor_stand] nbplayers = abandons nbplayers run tellraw @a ["",{"text":"[Lucky Block] ","color":"dark_red"},{"text":"All players finished the game! Congratulations to the winner: ","color":"gold"},{"selector":"@p[tag=winner]","color":"dark_red"},{"text":" !!!","color":"gold"}]

scoreboard players add @p[tag=winner] win 1
execute as @p[tag=winner] if score @s win matches 5.. run advancement grant @p[tag=winner] only lb:lucky_block/win1
execute as @p[tag=winner] if score @s win matches 20.. run advancement grant @p[tag=winner] only lb:lucky_block/win2
execute as @p[tag=winner] if score @s win matches 50.. run advancement grant @p[tag=winner] only lb:lucky_block/win3
